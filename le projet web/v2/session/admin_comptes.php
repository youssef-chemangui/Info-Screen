...
<?php
        ...
        if (isset($_SESSION['role'])) {
            if ($_SESSION['role'] == 'G') {
                ...
                echo ("<br />");
                $requete1="SELECT * FROM t_profil_pfl JOIN t_compte_cpt USING (cpt_pseudo);";
                ...
                if ($resultat1 == false) {
                    ...
                    echo("</tr>");
                    echo "<br />";
                    echo "<p>Le nombre total des comptes: $resultat1->num_rows </p>";
                    echo "<br />";

                    while ($prof=$resultat1->fetch_assoc()){
                        echo ("<tr>");
                        echo ("<td>"); 
                        echo ($prof['cpt_pseudo']); 
                        echo ("</td>"); 
                        echo ("<td>"); 
                        echo ($prof['pfl_nom']); 
                        echo ("</td>"); 
                        echo ("<td>"); 
                        echo ($prof['pfl_prenom']); 
                        echo ("</td>"); 
                        echo ("<td>"); 
                        echo ($prof['pfl_role']); 
                        echo ("</td>"); 
                        echo ("<td>"); 
                        echo ($prof['pfl_validite']); 
                        echo ("</td>"); 
                        echo ("<td>");
                            echo "<form action='comptes_action.php' method='POST'>"; 
                            echo "<input type='hidden' name='pseudosel' value='".$prof['cpt_pseudo']."'/>";
                            echo "<button type='submit' class='btn btn-default'
                            name='desactive'>Activer/Désactiver</button>";
                        echo "</form>";
                        echo ("</td>");
                        echo ("</tr>");
                    } 
